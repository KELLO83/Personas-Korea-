...
def _load_to_neo4j(df: pl.DataFrame, reset: bool, batch_size: int) -> None:
    ...
    try:
        ...
        if reset:
            logger.info("???  기존 그래프 데이터를 삭제합니다 (적재 직전 초기화)...")
...
def _batch_create_hobby_relationships(loader, reset: bool, batch_size: int = 5000) -> None:
    ...
    if reset:
        logger.info("???  기존 취미(Hobby) 노드와 취미 관계(LIKES, ENJOYS_HOBBY)를 초기화합니다...")
        with loader.driver.session(database=loader.database) as session:
...
def _first_existing_path(candidate_paths: tuple[str, ...]) -> str:
...
def _run_gds_pipeline(top_k: int) -> None:
    script_path = os.path.join(PROJECT_ROOT, "ops", "graph", "build_gds.py")
