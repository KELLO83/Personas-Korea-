# PRD: GNN Hobby Recommendation Engine v2.7
...
> **v2.7 상태**: Phase 2 PROMOTED (LightGBM), Phase 2.5 closed, Phase 3 NO-GO (MMR + category one-hot), Phase 5-A NO-GO (KURE MMR blocked).
...
### 1.2 현재 상태 (v2.7 — Phase 2 PROMOTED, Phase 2.5 closed, Phase 3 NO-GO, Phase 5 NO-GO)
...
| Diversity | `category_diversity_reward=0`, MMR **NO-GO** | Phase 3에서 category one-hot MMR 실험 → diversity gate 미충족. Phase 5-KURE MMR 실험도 validation에서 recall/ndcg gate 미통과하여 **NO-GO**. `--use-mmr` 플래그로만 제공 (기본값 false) |
...
│   → Phase 5에서 KURE dense embedding으로 재평가 완료(2026-05-01) │
└─────────────────────────────────────────────────────────────┘
      │
      ▼
[Final Top-K Output]
...
| Diversity 알고리즘 | **MMR with dense embeddings** (재평가 완료, 현재 default disabled) | Phase 3에서 category one-hot MMR → NO-GO (binary cosine similarity). Phase 5-KURE 재평가도 NO-GO (`lambda 0.5/0.7/0.8/0.9`). `--use-mmr` 플래그로만 제공 (기본값 false). |
...
- [x] `recommend_for_persona.py`에 모든 플래그 통합 (`--use-learned-ranker`, `--use-mmr`, `--mmr-lambda`, `--explain`, `--use-text-embedding`)
- [x] `evaluate_reranker.py`에 모든 모드 통합 (deterministic / learned / learned+mmr)
- [x] `phase5_kure_mmr_lambda_<lambda>` validation sweep 실행 (0.5/0.7/0.8/0.9)
- [ ] 전체 ablation 실행 (Phase 6.2: cold-start/segment)는 미실행
- [x] `experiment_decisions.json`에 최종 결과 기록
- [x] `experiment_run_summary.md` 업데이트
- [x] `README.md`에 v2 아키텍처 반영
...
[GO]  모든 ablation 결과가 promotion gate 기준 충족
 ...
 [NO-GO] promotion gate 미충족 OR regression 발견
        → v2 LightGBM은 Stage1 collapse 완화되지 않았으나, default는 유지
        → KURE MMR은 validation 미통과로 `--use-learned-ranker` default는 변경 없음
        → v2는 --use-learned-ranker flag로만 제공 (MMR는 optional)
```
...
| 2026-05-01 | v2.7 | Phase 5-A KURE dense embedding MMR 실행 결과 반영. lambda 0.5/0.7/0.8/0.9 모두 NO-GO (validation gate 실패)로 test 생략 및 기본 `MMR=false` 유지. | 구현 검증 |
...
**작성일**: 2026-05-01  
**버전**: v2.7 (A-단계형)  
**상태**: Phase 2 PROMOTED, Phase 2.5 closed, Phase 3 NO-GO (MMR one-hot 한계), Phase 5 NO-GO (KURE MMR blocked)
**다음 단계**: Phase 5 결과 반영 후 텍스트 임베딩 leakage-safe ablation 및 ranking 품질 개선 전략 검토
