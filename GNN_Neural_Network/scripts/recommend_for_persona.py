...
from GNN_Neural_Network.gnn_recommender.rerank import RerankedCandidate, build_reranker_config, merge_stage1_candidates, rerank_candidates  # noqa: E402
...
def main() -> None:
    ...
    normalization_method = _normalization_method(config.paths.score_normalization)
    pool_k = max(args.top_k, config.rerank.candidate_pool_size) if args.rerank else args.top_k
    provider_candidates = {
    ...
    fallback_usage = _fallback_usage_record(
        ...
        lightgcn_count=len(provider_candidates["lightgcn"]),
