...
def test_phase23_decision_file_has_promoted_lightgbm_defaults() -> None:
    ...
    assert stage1["status"] == "selected_baseline_and_default"
    assert stage1["selected_baseline"] == ["popularity", "cooccurrence"]

    assert stage2["status"] == "promoted_current_sota_and_production_default"
    assert stage2["strategy"] == (
        "LightGBM learned ranker with E5-small-ko-v2 single + domain-specific text similarities"
    )
    ...
    assert any(
        ...
        and "E5-domain rank/margin feature validation" in str(item.get("task", ""))
