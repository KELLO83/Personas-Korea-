...
CPU_TRAIN_BATCH_SIZE = 4096
MAX_AUTO_TRAIN_BATCH_SIZE = 65536
MIN_AUTO_TRAIN_BATCH_SIZE = 1024
...
def train_lightgcn(indexed: IndexedEdges, split: EdgeSplit, config: LightGCNConfig) -> dict[str, Any]:
    ...
    train_batch_size = _resolve_train_batch_size(config, device)
    ...
    metrics_payload = {
        ...
        "train_batch_mode": "auto" if int(config.train.batch_size) <= 0 else "manual",
        "train_vram_fraction": float(config.train.vram_fraction),
    }
    save_json(config.paths.metrics, metrics_payload)
    return metrics_payload


def _resolve_train_batch_size(config: LightGCNConfig, device: torch.device) -> int:
    requested = int(config.train.batch_size)
    if requested > 0:
        return requested
    if device.type != "cuda":
        return CPU_TRAIN_BATCH_SIZE

    free_bytes, _total_bytes = torch.cuda.mem_get_info(device)
    vram_fraction = max(0.1, min(float(config.train.vram_fraction), 0.9))
    usable_bytes = int(free_bytes * vram_fraction)
    bytes_per_sample = max(1, config.train.embedding_dim * 4 * 6)
    auto_batch_size = usable_bytes // bytes_per_sample
    return max(MIN_AUTO_TRAIN_BATCH_SIZE, min(MAX_AUTO_TRAIN_BATCH_SIZE, int(auto_batch_size)))
