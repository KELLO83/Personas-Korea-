...
class PersonEmbeddingCache:
    ...
    def __init__(
        ...
        *,
        ...
        batch_size: int = 0,
        vram_fraction: float = DEFAULT_TEXT_EMBEDDING_VRAM_FRACTION,
        ...
        self.batch_size = _resolve_text_embedding_batch_size(
            requested=int(batch_size),
            device=self.device,
            vram_fraction=float(vram_fraction),
        )
        self.batch_mode = "auto" if int(batch_size) <= 0 else "manual"
        self.vram_fraction = float(vram_fraction)
...
class HobbyEmbeddingCache:
    ...
    def encode(self, hobby_name: str) -> np.ndarray:
        ...
        emb = model.encode(hobby_name, convert_to_numpy=True, show_progress_bar=False, batch_size=self.batch_size)
