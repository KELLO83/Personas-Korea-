...
from experiments.hobby_recommender_ml.hobby_recommender.embedding_cache import HobbyEmbeddingCache, PersonEmbeddingCache  # noqa: E402
...
from experiments.hobby_recommender_ml.hobby_recommender.ranker import LightGBMRanker, RANKER_BASE_FEATURE_COLUMNS  # noqa: E402
from experiments.hobby_recommender_ml.hobby_recommender.text_embedding import KURE_MODEL_NAME, mask_holdout_hobbies, post_mask_leakage_audit  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=Path, default=Path("experiments/hobby_recommender_ml/configs/lightgcn_hobby.yaml"))
    parser.add_argument(
        "--sota-dir",
        type=Path,
        default=Path("experiments/hobby_recommender_ml/artifacts/experiments/phase2_5_num_leaves_31"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("experiments/hobby_recommender_ml/artifacts/experiments/phase5_c_text_embedding/sota_pool_kure_stage2_feature"),
    )
    ...
    parser.add_argument("--text-embedding-batch-size", type=int, default=0)
    parser.add_argument("--text-embedding-vram-fraction", type=float, default=DEFAULT_TEXT_EMBEDDING_VRAM_FRACTION)
...
def main() -> None:
    ...
    batch_size = _resolve_text_embedding_batch_size(
        requested=int(args.text_embedding_batch_size),
        device=device,
        vram_fraction=float(args.text_embedding_vram_fraction),
    )
...
def _find_feature_cache(sota_dir: Path, split: str) -> Path:
    cache_dirs = [
        ...
        Path("experiments/hobby_recommender_ml/artifacts/cache/cache"),
    ...
    *,
    ...
    return {
        ...
        "resource_plan": {
            ...
            "text_embedding_batch_mode": "auto" if int(args.text_embedding_batch_size) <= 0 else "manual",
            "text_embedding_vram_fraction": float(args.text_embedding_vram_fraction),
        },
...
def _resolve_text_embedding_batch_size(*, requested: int, device: str, vram_fraction: float) -> int:
    if requested > 0:
        return requested
    if device != "cuda":
    ...
    usable_mb = int((free_bytes / 1024 / 1024) * max(0.1, min(vram_fraction, 0.9)))
