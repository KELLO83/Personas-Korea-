...
from experiments.hobby_recommender_ml.hobby_recommender.rerank import HobbyCandidate, build_reranker_config  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="KURE batch size. Default 0 auto-sizes from available GPU VRAM.",
    )
    parser.add_argument(
        "--text-embedding-vram-utilization",
        "--embedding-vram-utilization",
        "--text-embedding-vram-fraction",
...
def _resolve_text_embedding_resource_plan(args: argparse.Namespace, device: str) -> dict[str, object]:
    ...
    if requested_target_mb > 0:
        ...
        usable_mb = max(512, int(free_mb * max(0.1, min(float(args.text_embedding_vram_utilization), 0.9))))
        mode = "auto_vram"
