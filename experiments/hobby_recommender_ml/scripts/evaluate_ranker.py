...
from experiments.hobby_recommender_ml.hobby_recommender.baseline import (  # noqa: E402
    build_cooccurrence_counts,
    build_popularity_counts,
)
from experiments.hobby_recommender_ml.hobby_recommender.config import load_config, validate_experimental_feature_policy  # noqa: E402
from experiments.hobby_recommender_ml.hobby_recommender.data import (
    LEAKAGE_TEXT_FIELDS,
    PersonContext,
...
from experiments.hobby_recommender_ml.hobby_recommender.embedding_cache import (
    HobbyEmbeddingCache,
    PersonEmbeddingCache,
)  # noqa: E402
from experiments.hobby_recommender_ml.hobby_recommender.metrics import summarize_ranking_metrics  # noqa: E402
from experiments.hobby_recommender_ml.hobby_recommender.phase6 import build_stage2_cross_features  # noqa: E402
from experiments.hobby_recommender_ml.hobby_recommender.diversity import (
    compute_hobby_embeddings,
    dpp_rerank,
    mmr_rerank,
)
from experiments.hobby_recommender_ml.hobby_recommender.ranker import (
    LightGBMRanker,
...
from experiments.hobby_recommender_ml.hobby_recommender.text_embedding import (  # noqa: E402
...
from experiments.hobby_recommender_ml.hobby_recommender.rerank import (  # noqa: E402
...
PHASE5_BASELINE_PATHS = {
    "validation": Path("experiments/hobby_recommender_ml/artifacts/experiments/phase2_5_num_leaves_31/validation_metrics.json"),
    "test": Path("experiments/hobby_recommender_ml/artifacts/experiments/phase2_5_num_leaves_31/test_metrics.json"),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Batch size for KURE embeddings. Default 0 auto-sizes from available GPU VRAM.",
    )
    parser.add_argument(
        "--embedding-vram-utilization",
        "--embedding-vram-fraction",
        type=float,
        default=0.9,
...
def main() -> None:
    ...
    model_path = args.model_path or Path("experiments/hobby_recommender_ml/artifacts/ranker_model.txt")
    ...
    if args.use_mmr or args.use_dpp:
        if args.mmr_embedding_method == "kure":
            ...
            mmr_device = "cuda" if _torch_module().cuda.is_available() else "cpu"
            ...
            mmr_embedding_meta = {
                ...
                "attention_implementation": str(mmr_embedding_meta.get("attention_implementation", text_embedding_attention_implementation)),
                "torch_dtype": str(mmr_embedding_meta.get("torch_dtype", text_embedding_torch_dtype)),
                "torch_compile": bool(mmr_embedding_meta.get("torch_compile", text_embedding_torch_compile)),
                "torch_compile_mode": str(mmr_embedding_meta.get("torch_compile_mode", text_embedding_torch_compile_mode)),
                "batch_size": mmr_embedding_batch_size,
    ...
    if (include_text_embedding_feature or include_domain_text_embedding_features or include_text_rank_margin_features) and not feature_cache_hit:
        ...
        text_embedding_audit["backend_policy"] = embedding_backend_policy
        LOGGER.info(
    ...
    embedding_metadata_enabled = text_embedding_enabled or args.stage1_kure_semantic_provider or args.stage1_quota_experiment == "e5_semantic" or (
        (args.use_mmr or args.use_dpp) and args.mmr_embedding_method == "kure"
    )
    embedding_model_metadata = _embedding_model_metadata(
        enabled=embedding_metadata_enabled,
        model_name=text_embedding_model_name,
        model_revision=text_embedding_model_revision,
        ...
        attention_implementation=text_embedding_attention_implementation if embedding_metadata_enabled else "",
        torch_dtype=text_embedding_torch_dtype if embedding_metadata_enabled else "",
        torch_compile=text_embedding_torch_compile if embedding_metadata_enabled else False,
        torch_compile_mode=text_embedding_torch_compile_mode if embedding_metadata_enabled else "",
        backend_policy=embedding_backend_policy if embedding_metadata_enabled else {},
    )
    ...
    output_path = args.output or Path("experiments/hobby_recommender_ml/artifacts/ranker_eval_metrics.json")
        ...
        json.dumps(
            {
                ...
                "text_embedding": {
                    ...
                    "attention_implementation": str(
                        getattr(args, "text_embedding_attn_implementation", DEFAULT_ATTENTION_IMPLEMENTATION)
                        or DEFAULT_ATTENTION_IMPLEMENTATION
                    )
                    if _feature_policy(feature_columns)["include_text_embedding_feature"]
                    else "",
                    "torch_dtype": str(getattr(args, "text_embedding_torch_dtype", DEFAULT_TORCH_DTYPE) or DEFAULT_TORCH_DTYPE)
                    if _feature_policy(feature_columns)["include_text_embedding_feature"]
                    else "",
                    "torch_compile": bool(getattr(args, "text_embedding_torch_compile", DEFAULT_TORCH_COMPILE))
                    if _feature_policy(feature_columns)["include_text_embedding_feature"]
                    else False,
                    "torch_compile_mode": str(
                        getattr(args, "text_embedding_torch_compile_mode", DEFAULT_TORCH_COMPILE_MODE)
                        or DEFAULT_TORCH_COMPILE_MODE
                    )
                    if _feature_policy(feature_columns)["include_text_embedding_feature"]
                    else "",
                    "preprocessing_version": TEXT_EMBEDDING_PREPROCESSING_VERSION
...
def _resolve_embedding_resource_plan(args: argparse.Namespace) -> dict[str, object]:
    ...
    if requested_target_mb > 0:
        usable_mb = min(requested_target_mb, max(512, int(total_mb * 0.9)))
        mode = "auto_target_vram"
    else:
        usable_mb = max(512, int(free_mb * max(0.1, min(float(args.embedding_vram_utilization), 0.9))))
        mode = "auto_vram"
