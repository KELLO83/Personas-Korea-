...
gnn_root = os.path.dirname(script_dir)                  # experiments/hobby_recommender_ml/
project_root = os.path.dirname(os.path.dirname(gnn_root))  # Workspace Root (Nemotron-Personas-Korea)

os.chdir(gnn_root)  # 작업 디렉토리를 experiments/hobby_recommender_ml/로 변경
...
def simulate_masking_and_local_embedding():
    ...
    cache = HobbyEmbeddingCache("experiments/hobby_recommender_ml/artifacts/embeddings_cache/local_test_cache.txt")
...
def simulate_global_feature_builder():
    ...
    try:
        from experiments.hobby_recommender_ml.hobby_recommender.data import build_text_embedding_features
        result = build_text_embedding_features(
            train_edges=[(0, 0), (1, 2)],
            person_to_id=person_to_id,
            hobby_to_id=hobby_to_id,
            contexts=contexts,
            embedding_cache_dir="experiments/hobby_recommender_ml/artifacts/embeddings_cache"
        )
        ...
        print(f"??  모듈 임포트 또는 연동 중 에러 발생 (빌드 환경 특성): {e}")
        print("   (이것은 구조적 임포트 이슈일 수 있으나 코드 로직 자체는 유효함)")


def main():
    print("\n" + "#"*60)
    print("# hobby_recommender_ml Phase 5-B: Text Embedding Pipeline")
